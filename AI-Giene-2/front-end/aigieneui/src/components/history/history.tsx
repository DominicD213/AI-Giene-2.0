...
const History: React.FC = () => {
    ...
    const queries = useSelector((state: RootState) => state.query.queries); // Fix: Select queries from the correct slice
    const dispatch = useDispatch();
    const queriesEndRef = useRef<HTMLDivElement>(null);  // Reference to the end of the list for scrolling effect
    const { fetchQueriesHistory } = useFetchQueriesHistory(); // Fetch queries using custom hook
    ...
    }, [searchHistory]); // Fetch queries when searchHistory changes
    ...
    return (
        <div>
            <div className='ml-5'>
                ...
                <div className="space-y-4 flex flex-col mx-5 mt-10 rounded overflow-auto max-h-[50vh] scrollbar-hide">
                    {queries.length > 0 && searchHistory !== '' ? (
