...
import useCheckSessionStatus from "../../hooks/authentication/authenticationService";
import useFetchQueries from "../../hooks/sessionQuery/fetchQueries";  
import { Query } from "../../store/session/sessionStatus";
import { changeloadingState } from "../../store/session/sessionStatus";
import useSocketEvents from "../../hooks/sessionQuery/socket";
...
const SessionQuery: React.FC = () => {
    ...
    useEffect(() => {
        const fetchAndSetLoading = async () => {
            if (sessionStatus === true && !loaded) {
                await fetchQueries();
                dispatch(changeloadingState());
            }
        };
        fetchAndSetLoading();
    }, [fetchQueries, sessionStatus, loaded, dispatch]);
